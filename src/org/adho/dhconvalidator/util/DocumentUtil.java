...
public class DocumentUtil {
   ...
   * @throws IllegalStateException if there is no such match
   */
  public static Element getFirstMatch(Document document, String query, XPathContext xPathContext) {
   ...
   * @throws IllegalStateException if there is no such match
   */
  public static Element getFirstMatch(
      Element subtreeRoot, String query, XPathContext xPathContext) {
   ...
   */
  public static Element tryFirstMatch(Document document, String query, XPathContext xPathContext) {
    Nodes nodes = document.query(query, xPathContext);
    if ((nodes.size() > 0) && (nodes.get(0) instanceof Element)) {
      return (Element) nodes.get(0);
    } else {
   ...
   * @return the first match or <code>null</code> if there is no match
   */
  public static Element tryFirstMatch(
      Element subtreeRoot, String query, XPathContext xPathContext) {
    Nodes nodes = subtreeRoot.query(query, xPathContext);
    if ((nodes.size() > 0) && (nodes.get(0) instanceof Element)) {
      return (Element) nodes.get(0);
   ...
   */
  public static boolean hasMatch(Element subtreeRoot, String query, XPathContext xPathContext) {
