...
public enum PropertyKey {
  ...
  base_url,
  /** <code>true</code> -> use DHConvalidator address generation with email for HTML output. */
  html_address_generation,
  /**
   * <code>true</code> -> a link to the corresponding (same name) xml file is added to the HTML
   * output.
   */
  html_to_xml_link,
  ...
  /**
   * @return an instance of UserProvider using the implementation specified with {@link
   *     #userProviderClass}.
  ...
  public static UserProvider getUserProviderInstance() throws IllegalStateException {
    try {
      Class<?> clazz =
          Class.forName(userProviderClass.getValue(), true, PropertyKey.class.getClassLoader());
  ...
  /**
   * @return an instance of PaperProvider using the implementation specified with {@link
   *     #paperProviderClass}.
   * @throws IllegalStateException in case of instantiation failure
   */
  public static PaperProvider getPaperProviderInstance() throws IllegalStateException {
    try {
      Class<?> clazz =
          Class.forName(paperProviderClass.getValue(), true, PropertyKey.class.getClassLoader());
